...
input = open("Day 22 Part 3.txt").read()
stuff = input.split("\n\n")

def solve(board, instructions):
    ...
    return 1000 * final_y + 4 * final_x + ((3 + sum(direction)) % 4)
        
def Part_3():
    total = 0
    for i in range(0, len(stuff) - 1, 2):
        board = stuff[i].split("\n")
        instructions = findall(r"(\d+|L|R)", stuff[i + 1])
        total += solve(board, instructions)
